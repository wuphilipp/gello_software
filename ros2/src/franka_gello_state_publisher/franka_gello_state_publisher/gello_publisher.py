...
class GelloPublisher(Node):
    def __init__(self):
        ...
        self.com_port = (
            self.get_parameter("com_port").get_parameter_value().string_value
        )
        ...
        self.robot_joint_publisher = self.create_publisher(
    ...
    def determine_default_com_port(self) -> str:
        ...
        if matches:
            ...
            self.get_logger().warn(
                "No com_ports detected. Please specify the com_port manually."
            )
    ...
    def get_values_from_config(self, config_file: str):
        ...
        self.gripper_range_rad: Tuple[float, float] = config[self.port][
            "gripper_range_rad"
        ]
    ...
    def publish_joint_jog(self):
        ...
        current_joints_corrected = (
            current_robot_joints - self.best_offsets
        ) * self.joint_signs
        ...
        robot_joint_states.position = [
            float(joint) for joint in current_joints_corrected
        ]

        gripper_joint_states = Float32()
        if self.gripper:
            gripper_position = current_joints[-1]
            gripper_joint_states.data = self.gripper_readout_to_percent(
                gripper_position
            )
        else:
    ...
    def add_dynamixel_driver_path(self):
        gello_path = os.path.abspath(
            os.path.join(
                get_package_prefix("franka_gello_state_publisher"), "../../../"
            )
        )
